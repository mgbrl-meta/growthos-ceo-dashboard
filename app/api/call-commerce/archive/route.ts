...
import { archiveEligibleLeads, listLeads } from '@/lib/call-commerce/repository';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('CALL_COMMERCE_ARCHIVE_ERROR', error);
...
export async function POST(request: NextRequest) {
  try {
    const access = await requireGrowthOSApiAccess(request);
    await archiveEligibleLeads(access.workspaceId, access.brandId);
