...
if (!PROJECT_ID) {
  ...
  throw new Error(
    'SHOPIFY_BULK_WAREHOUSE_PROJECT_MISSING'
  );
...
function requireTableId(
  ...
  if (
    ...
    throw new Error(
      'SHOPIFY_BULK_WAREHOUSE_STAGE_TABLE_INVALID'
    );
...
function normalizeInput(
  ...
  if (
    ...
    throw new Error(
      'SHOPIFY_BULK_WAREHOUSE_TENANT_IDENTITY_MISSING'
    );
...
// Object keys are recursively alphabetically sorted.
// ============================================================
...
// READ-ONLY.
//
// This is the canonical compatibility gate.
//
// For our existing 18,111-order test file:
//
// rawHashPresent should be 18,111.
// rawHashMissing must be 0.
//
// No RAW/STATE changes occur here.
// ============================================================

export async function analyzeShopifyBulkStage(
  rawInput
) {
  ...
  const [
    ...
    await bigquery.query({
      ...
      query: `
        ...
        FROM
          \`${PROJECT_ID}.${DATASET_ID}.shopify_orders_raw_json\`
  ...
  return {
    ...
    canonicalContract:
      'shopify_order_canonical_sha256_v1',
...
// compare STATE
//   ↓
// append unseen versions → RAW
//   ↓
// MERGE STATE
//   ↓
// CURRENT VIEW follows automatically
//
// IMPORTANT:
//
// No direct write to shopify_orders_current.
// ============================================================

export async function writeShopifyBulkStage(
  rawInput
) {
  ...
  const [
    ...
    await bigquery.query({
      ...
      query: `
        ...
        --
        -- Do not insert the same payload version twice.
        --
        -- This is particularly important for historical replay
        -- when STATE may already have advanced to a later
        -- Shopify order version.
        -- ====================================================
        ...
        -- MERGE STATE
        --
        -- This condition intentionally matches the existing
        -- shopify-writer.js behavior.
        -- ====================================================
...
export const SHOPIFY_BULK_WAREHOUSE_CONFIG = {
