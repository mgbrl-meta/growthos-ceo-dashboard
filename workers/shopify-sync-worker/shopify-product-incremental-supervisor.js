...
import {
  ensureShopifyProductsIncrementalState,
  getShopifyProductsIncrementalState,
  claimShopifyProductsIncrementalDispatch,
  createShopifyProductsIncrementalRun,
  releaseShopifyProductsIncrementalDispatch,
  recoverStaleShopifyProductsIncrementalClaims,
...
if (!PROJECT_ID) {
  ...
  throw new Error(
    'SHOPIFY_PRODUCT_INCREMENTAL_SUPERVISOR_PROJECT_MISSING'
  );
...
function getTopicId() {
  ...
  const value =
    requireString(
      RAW_TOPIC_NAME,
      'SHOPIFY_PRODUCT_INCREMENTAL_SUPERVISOR_TOPIC_MISSING'
    );
...
async function publishIncrementalJob(
  ...
  const job = {
    ...
    requestedBy:
      'shopify_product_incremental_supervisor',
...
async function dispatchAccount(
  ...
  // ==========================================================
  // ENSURE CONNECTION-SPECIFIC PRODUCTS STATE
  //
  ...
  await ensureShopifyProductsIncrementalState({
  ...
  const state =
    await getShopifyProductsIncrementalState({
  ...
  // dispatching Products for the same Shopify connection.
  // ==========================================================

  const claim =
    await claimShopifyProductsIncrementalDispatch({
  ...
  try {
    ...
    await createShopifyProductsIncrementalRun({
    ...
    try {
      ...
      await releaseShopifyProductsIncrementalDispatch({
      ...
      console.error(
        'SHOPIFY_PRODUCT_INCREMENTAL_SUPERVISOR_RELEASE_FAILED',
        {
...
// ============================================================
// SUPERVISE SHOPIFY PRODUCTS INCREMENTAL
//
// One invocation:
//
// 1. discover READY Shopify accounts
//
// 2. ensure per-connection Products state
//
...
export async function superviseShopifyProductsIncremental() {
  ...
  const recovery =
    await recoverStaleShopifyProductsIncrementalClaims();
      ...
      results.push({
        ...
        error:
          String(
            error?.message
            ||
            'SHOPIFY_PRODUCT_INCREMENTAL_SUPERVISOR_ACCOUNT_FAILED'
          ),
