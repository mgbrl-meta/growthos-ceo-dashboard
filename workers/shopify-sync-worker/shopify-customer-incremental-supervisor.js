...
import {
  ensureShopifyCustomersIncrementalState,
  getShopifyCustomersIncrementalState,
  claimShopifyCustomersIncrementalDispatch,
  createShopifyCustomersIncrementalRun,
  releaseShopifyCustomersIncrementalDispatch,
  recoverStaleShopifyCustomersIncrementalClaims,
...
if (!PROJECT_ID) {
  ...
  throw new Error(
    'SHOPIFY_CUSTOMER_INCREMENTAL_SUPERVISOR_PROJECT_MISSING'
  );
...
function getTopicId() {
  ...
  const value =
    requireString(
      RAW_TOPIC_NAME,
      'SHOPIFY_CUSTOMER_INCREMENTAL_SUPERVISOR_TOPIC_MISSING'
    );
...
async function publishIncrementalJob(
  ...
  const job = {
    ...
    requestedBy:
      'shopify_customer_incremental_supervisor',
...
async function dispatchAccount(
  ...
  // ==========================================================
  // ENSURE CONNECTION-SPECIFIC CUSTOMERS STATE
  //
  ...
  await ensureShopifyCustomersIncrementalState({
  ...
  const state =
    await getShopifyCustomersIncrementalState({
  ...
  // dispatching Customers for the same Shopify connection.
  // ==========================================================

  const claim =
    await claimShopifyCustomersIncrementalDispatch({
  ...
  try {
    ...
    await createShopifyCustomersIncrementalRun({
    ...
    //
    // queued/running run â†’ failed
    // state              â†’ failed
    // watermark          â†’ unchanged
    // retry              â†’ later
    // ========================================================

    try {

      await releaseShopifyCustomersIncrementalDispatch({
      ...
      console.error(
        'SHOPIFY_CUSTOMER_INCREMENTAL_SUPERVISOR_RELEASE_FAILED',
        {
...
// ============================================================
// SUPERVISE SHOPIFY CUSTOMERS INCREMENTAL
//
// One invocation:
//
// 1. discover READY Shopify accounts
//
// 2. ensure per-connection Customers state
//
...
//
// queued â†’ running
// success â†’ watermark advance
// failure â†’ retain watermark
//
// wiring comes in the NEXT step.
// ============================================================

export async function superviseShopifyCustomersIncremental() {
  ...
  const recovery =
    await recoverStaleShopifyCustomersIncrementalClaims();
      ...
      results.push({
        ...
        error:
          String(
            error?.message
            ||
            'SHOPIFY_CUSTOMER_INCREMENTAL_SUPERVISOR_ACCOUNT_FAILED'
          ),
