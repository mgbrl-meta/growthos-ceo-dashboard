...
if (!PROJECT_ID) {
  ...
  throw new Error(
    'SHOPIFY_PRODUCT_BULK_WAREHOUSE_PROJECT_MISSING'
  );
...
const CANONICAL_CONTRACT =
  'shopify_product_canonical_sha256_v1';
...
function requireTableId(
  ...
  if (
    ...
    throw new Error(
      'SHOPIFY_PRODUCT_BULK_STAGE_TABLE_INVALID'
    );
...
function normalizeInput(
  ...
  if (
    ...
    throw new Error(
      'SHOPIFY_PRODUCT_BULK_WAREHOUSE_TENANT_IDENTITY_MISSING'
    );
...
// Product warehouse accidentally.
// ============================================================
...
async function validateStage(
  ...
  if (
    ...
    throw new Error(
      'SHOPIFY_PRODUCT_BULK_WAREHOUSE_INVALID_JSON'
    );
  ...
  if (
    ...
    throw new Error(
      'SHOPIFY_PRODUCT_BULK_WAREHOUSE_PRODUCT_ID_MISSING'
    );
  ...
  if (
    result.invalidProductId !==
      0
  ) {

    throw new Error(
      'SHOPIFY_PRODUCT_BULK_WAREHOUSE_ENTITY_MISMATCH'
    );
  ...
  if (
    ...
    throw new Error(
      'SHOPIFY_PRODUCT_BULK_WAREHOUSE_DUPLICATE_PRODUCT_IDS'
    );
...
// used by shopify-product-writer.js:
//
...
// SHA256
//
// Therefore a Product already written manually will have the
...
export async function analyzeShopifyProductBulkStage(
  rawInput
) {
  ...
  const [
    ...
    await bigquery.query({
      ...
      query: `
        ...
        FROM
          \`${PROJECT_ID}.${DATASET_ID}.shopify_products_raw_json\`
...
// ============================================================
// WRITE PRODUCT BULK STAGE
//
// HIGH-VOLUME HISTORICAL PRODUCT PATH
//
// STAGE
//   â†“
// canonicalize
...
// SHA256
//   â†“
// compare Product STATE
//   â†“
// append unseen versions â†’ Product RAW
//   â†“
// MERGE Product STATE
//   â†“
// shopify_products_current follows automatically
//
// IMPORTANT:
//
...
// backwards when a newer realtime/manual Product already
// exists.
// ============================================================

export async function writeShopifyProductBulkStage(
  rawInput
) {
  ...
  const [
    ...
    await bigquery.query({
      ...
      query: `
        ...
        --
        -- Historical replay may encounter a Product version
        ...
        -- Product writer.
        --
        -- A historical Product version must never replace a
        -- newer realtime Product version.
        -- ====================================================
...
export const SHOPIFY_PRODUCT_BULK_WAREHOUSE_CONFIG = {
